/**
 * CHROMA TEXT STUDIO - MAIN APP COORDINATOR
 * Coordinates user input, visual styling, font loading, canvas scaling, and video rendering.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function scaleCanvas() {
        const width = parseInt(canvasWidth.value) || 1920;
        const height = parseInt(canvasHeight.value) || 1080;
    ...
    function applyStylesAndPlay() {
        ...
        if (strokeW > 0) {
            animatedText.style.webkitTextStroke = `${strokeW}px ${strokeColor.value}`;
        } else {
        ...
        if (enableShadow.checked) {
            ...
            const offsetX = parseInt(shadowOffsetX.value) ?? 4;
            const offsetY = parseInt(shadowOffsetY.value) ?? 4;
    ...
    canvasWidth.addEventListener('change', () => {
        // Sync height if predefined ratio is selected
        if (canvasWidth.value === '1080') {
            if (canvasHeight.value !== '1080' && canvasHeight.value !== '1920') {
    ...
    btnRecord.addEventListener('click', () => {
        ...
        const exportConfig = {
            width: parseInt(canvasWidth.value) || 1920,
            height: parseInt(canvasHeight.value) || 1080,
            bgType: bgType.value,
